...
`data/raw_csv/` に蓄積するGarminデータは、解析の再現性と**気象実績データ（気温・湿度等）との正確な照合**を行うため、以下の形式で統一します。
...
### Examples
- `202605061830_E-jog_inda-home.csv`
- `202605150600_T-run_kaike.csv`
...
- **二重カウントの防止:** 合計距離や平均ペースを算出する際は、この最終行を計算対象から除外するか、合計値のみを使用するようにしてください。
- **時刻の重要性:** 開始時刻（HHMM）を含めることで、気象庁の時別データと突合し、環境負荷の正確な分析を可能にする。
